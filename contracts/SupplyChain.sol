...
contract SupplyChain {
  ...
  modifier verifyCaller (address _address) {
    require (msg.sender == _address, "Not the same");
    _;
  }
  ...
  modifier forSale(uint _sku) {
    require(items[_sku].state == State.ForSale && bytes(items[_sku].name).length > 0, "Sorry but you are not able to sale it.");
    _;
  }
  ...
  modifier shipped(uint _sku){
    require(items[_sku].state == State.Shipped, "The items is not shipped");
    _;
  }
